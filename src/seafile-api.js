...
class SeafileAPI {
   ...
   */
  login() {
    //console.log("function login", this);
    const url = this.server + "/api2/auth-token/";
    return axios
      .post(url, {
        username: this.username,
      ...
      .then((response) => {
        // console.log("function login axios", response);
        this.token = response.data.token;
        this.req = axios.create({
          baseURL: this.server,
          headers: { Authorization: "Token " + this.token },
        });
        //console.log("function login axios", this);
      });
  }

  authPing() {
    //console.log("function authPing", this);
  ...
  quitGroup(groupID, userName) {
    ...
    const url =
      this.server + "/api/v2.1/groups/" + groupID + "/members/" + name + "/";
    return this.req.delete(url);
  }
  ...
  importGroupMembersViaFile(groupID, file) {
    const url =
      this.server + "/api/v2.1/groups/" + groupID + "/members/import/";
  ...
  createGroupOwnedLibrary(groupID, repo) {
    ...
    const url =
      this.server + "/api/v2.1/groups/" + groupID + "/group-owned-libraries/";
    let form = new FormData();
  ...
  shareGroupOwnedRepoToUser(repoID, permission, username, path) {
    const url =
      this.server +
      "/api/v2.1/group-owned-libraries/" +
      repoID +
      "/user-share/";
    let form = new FormData();
    form.append("permission", permission);
  ...
  modifyGroupOwnedRepoUserSharedPermission(repoID, permission, username, path) {
    //need check
    const url =
      this.server +
      "/api/v2.1/group-owned-libraries/" +
  ...
  modifyGroupOwnedRepoGroupSharedPermission(repoID, permission, groupID, path) {
    //need check
    const url =
      this.server +
      "/api/v2.1/group-owned-libraries/" +
  ...
  deleteRepoShareLink(repoID, token) {
    const url =
      this.server + "/api/v2.1/repos/" + repoID + "/share-links/" + token + "/";
    return this.req.delete(url);
  }
  ...
  leaveShareGroupOwnedRepo(repoID) {
    const url =
      this.server +
      "/api/v2.1/group-owned-libraries/user-share-in-libraries/" +
      repoID +
      "/";
    return this.req.delete(url);
  }
  ...
  shareFolder(repoID, path, shareType, permission, paramArray) {
    ...
    const url =
      this.server + "/api2/repos/" + repoID + "/dir/shared_items/?p=" + path;
    return this.req.put(url, form);
  }
  ...
  listCustomPermissions(repoID) {
    const url =
      this.server + "/api/v2.1/repos/" + repoID + "/custom-share-permissions/";
    return this.req.get(url);
  }
  ...
  resetAndSendEncryptedRepoPassword(repoID) {
    const url =
      this.server + "/api/v2.1/repos/" + repoID + "/send-new-password/";
    let form = new FormData();
    return this._sendPostRequest(url, form);
  }
  ...
  selectOwnedRepoToPublic(repoID, options) {
  ...
  getDirInfo(repoID, dirPath) {
    const path = encodeURIComponent(dirPath);
    const url =
      this.server + "/api/v2.1/repos/" + repoID + "/dir/detail/?path=" + path;
    return this.req.get(url);
  }
  ...
  queryAsyncOperationProgress(task_id) {
    const url =
      this.server + "/api/v2.1/query-copy-move-progress/?task_id=" + task_id;
    return this.req.get(url);
  }
  ...
  queryZipProgress(zip_token) {
    const url =
      this.server + "/api/v2.1/query-zip-progress/?token=" + zip_token;
    return this.req.get(url);
  }
  ...
  getFileInfo(repoID, filePath) {
    const path = encodeURIComponent(filePath);
    const url =
      this.server + "/api2/repos/" + repoID + "/file/detail/?p=" + path;
    return this.req.get(url);
  }
  ...
  getFileDownloadLink(repoID, filePath) {
    ...
    const path = encodeURIComponent(filePath);
    const url =
      this.server + "/api2/repos/" + repoID + "/file/?p=" + path + "&reuse=1";
    return this.req.get(url);
  }
  ...
  getFileUploadedBytes(repoID, filePath, fileName) {
    let url =
      this.server + "/api/v2.1/repos/" + repoID + "/file-uploaded-bytes/";
    let params = {
      parent_dir: filePath,
      file_name: fileName,
    };
  ...
  listFileHistoryRecords(repoID, path, page, per_page) {
    const url =
      this.server + "/api/v2.1/repos/" + repoID + "/file/new_history/";
    const params = {
      path: path,
      page: page,
      per_page: per_page,
  ...
  listComments(repoID, filePath, resolved) {
    const path = encodeURIComponent(filePath);
    let url =
      this.server + "/api2/repos/" + repoID + "/file/comments/?p=" + path;
    if (resolved) {
  ...
  getCommentsNumber(repoID, path) {
    ...
    const url =
      this.server + "/api2/repos/" + repoID + "/file/comments/counts/?p=" + p;
    return this.req.get(url);
  }
  ...
  listFileTags(repoID, filePath) {
    ...
    var url =
      this.server + "/api/v2.1/repos/" + repoID + "/file-tags/?file_path=" + p;
    return this.req.get(url);
  }
  ...
  sysAdminUpdateAbuseReport(handled, abuseReportId) {
    const url =
      this.server + "/api/v2.1/admin/abuse-reports/" + abuseReportId + "/";
    let form = new FormData();
  ...
  searchUsers(searchParam) {
    const url =
      this.server + "/api2/search-user/?q=" + encodeURIComponent(searchParam);
    return this.req.get(url);
  }
  ...
  orgAdminListOrgGroups(orgID, page) {
    const url =
      this.server + "/api/v2.1/org/" + orgID + "/admin/groups/?page=" + page;
    return this.req.get(url);
  }
  ...
  orgAdminGetGroup(orgID, groupID) {
    const url =
      this.server + "/api/v2.1/org/" + orgID + "/admin/groups/" + groupID + "/";
    return this.req.get(url);
  }
  ...
  orgAdminDeleteOrgRepo(orgID, repoID) {
    const url =
      this.server + "/api/v2.1/org/" + orgID + "/admin/repos/" + repoID + "/";
    return this.req.delete(url);
  }
  ...
  orgAdminListDepartGroups(orgID) {
    const url =
      this.server + "/api/v2.1/org/" + orgID + "/admin/address-book/groups/";
    return this.req.get(url);
  }
  ...
  orgAdminListFileAudit(email, repoID, page) {
    let url =
      this.server + "/api/v2.1/org/admin/logs/file-access/?page=" + page;
    if (email) {
  ...
  orgAdminListFileUpdate(email, repoID, page) {
    let url =
      this.server + "/api/v2.1/org/admin/logs/file-update/?page=" + page;
    if (email) {
      url = url + "&email=" + encodeURIComponent(email);
    }
    if (repoID) {
  ...
  orgAdminListPermAudit(email, repoID, page) {
    let url =
      this.server + "/api/v2.1/org/admin/logs/repo-permission/?page=" + page;
    if (email) {
      url = url + "&email=" + encodeURIComponent(email);
    }
    if (repoID) {
  ...
  addRepoShareInvitations(repoID, path, emails, permission) {
    const url =
      this.server + "/api/v2.1/repos/" + repoID + "/shared/invitations/batch/";
    const data = {
      type: "guest",
      accepters: emails,
      path: path,
  ...
  updateRepoShareInvitation(repoID, path, token, permission) {
    const url =
      this.server + "/api/v2.1/repos/" + repoID + "/shared/invitation/";
  ...
  updateDepartmentRepoUserFolderPerm(
    repoID,
    permission,
    folderPath,
    userEmail
  ...
  deleteDepartmentRepoUserFolderPerm(
    repoID,
    permission,
    folderPath,
    userEmail
  ...
  adminListWorkWeixinDepartmentMembers(departmentID, params) {
    const url =
      this.server +
      "/api/v2.1/admin/work-weixin/departments/" +
      departmentID +
      "/members/";
    return this.req.get(url, { params: params });
  }
  ...
  adminListDingtalkDepartmentMembers(departmentID) {
    const url =
      this.server +
      "/api/v2.1/admin/dingtalk/departments/" +
      departmentID +
      "/members/";
    return this.req.get(url);
  }
  ...
  sysAdminGetRepoHistorySetting(repoID) {
    const url =
      this.server + "/api/v2.1/admin/libraries/" + repoID + "/history-limit/";
    return this.req.get(url);
  }
  ...
  sysAdminUpdateRepoSharedItemPermission(
    repoID,
    shareType,
    shareTo,
    permission
  ...
  sysAdminListRepoDirents(repoID, dir) {
    const url =
      this.server + "/api/v2.1/admin/libraries/" + repoID + "/dirents/";
    let params = {
      parent_dir: dir,
    };
    return this.req.get(url, { params: params });
  ...
  sysAdminDeleteRepoDirent(repoID, path) {
    const url =
      this.server + "/api/v2.1/admin/libraries/" + repoID + "/dirent/";
    let params = {
      path: path,
    };
  ...
  sydAdminGetSysRepoItemUploadURL(path) {
    const url =
      this.server + "/api/v2.1/admin/system-library/upload-link/?from=web";
    let params = {
      path: path,
    };
    return this.req.get(url, { params: params });
  ...
  sysAdminListGroupRepos(groupID) {
    const url =
      this.server + "/api/v2.1/admin/groups/" + groupID + "/libraries/";
    return this.req.get(url);
  }
  ...
  sysAdminGetDepartmentInfo(groupID, showAncestors) {
    let url =
      this.server + "/api/v2.1/admin/address-book/groups/" + groupID + "/";
  ...
  sysAdminListOrgUsers(orgID) {
    const url =
      this.server + "/api/v2.1/admin/organizations/" + orgID + "/users/";
    return this.req.get(url);
  }
  ...
  sysAdminListOrgGroups(orgID) {
    const url =
      this.server + "/api/v2.1/admin/organizations/" + orgID + "/groups/";
    return this.req.get(url);
  }

  sysAdminListOrgRepos(orgID) {
    const url =
      this.server + "/api/v2.1/admin/organizations/" + orgID + "/repos/";
    return this.req.get(url);
  }
  ...
  sysAdminListShareInRepos(receiverEmail) {
    const url =
      ...
      encodeURIComponent(receiverEmail) +
      "/beshared-repos/";
    return this.req.get(url);
  }
  ...
  sysAdminSetForceTwoFactorAuth(email, isForce2FA) {
    ...
    const url =
      this.server + "/api2/two-factor-auth/" + encodeURIComponent(email) + "/";
    let formData = new FormData();
  ...
  sysAdminUpdateUser(email, attribute, value) {
    const url =
      this.server + "/api/v2.1/admin/users/" + encodeURIComponent(email) + "/";
    let formData = new FormData();
    switch (attribute) {
      case "password":
  ...
  sysAdminDeleteInstitution(institutionID) {
    const url =
      this.server + "/api/v2.1/admin/institutions/" + institutionID + "/";
    return this.req.delete(url);
  }
  ...
  sysAdminListInstitutionUsers(institutionID, page, perPage) {
    const url =
      this.server + "/api/v2.1/admin/institutions/" + institutionID + "/users/";
    let params = {
      page: page,
      per_page: perPage,
    };
  ...
  sysAdminUpdateTermAndCondition(termID, name, versionNumber, text, isActive) {
    const url =
      this.server + "/api/v2.1/admin/terms-and-conditions/" + termID + "/";
    let formData = new FormData();
    formData.append("name", name);
    formData.append("version_number", versionNumber);
    formData.append("text", text);
  ...
  listVirusScanRecords(pageNum) {
    const url =
      this.server + "/api/v2.1/admin/virus-scan-records/?page=" + pageNum;
    return this.req.get(url);
  }
  ...
  deleteVirusScanRecord(virusID) {
    const url =
      this.server + "/api/v2.1/admin/virus-scan-records/" + virusID + "/";
    return this.req.delete(url);
  }
